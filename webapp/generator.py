...
from .template_data import template_bytes


def safe_filename(name: str):
    name = re.sub(r"[^A-Za-z0-9À-ž_-]+", "_", name).strip("_")
    return f"{name or 'Hotel'}_Canva.xlsx"


def _template_path() -> Path:
    # Write the embedded template to /tmp. This avoids Vercel include/static-file issues.
    path = Path(tempfile.gettempdir()) / "Webbeds_Hotel_Canvas.xlsx"
    if not path.exists():
...
def fill_canvas(contract: ContractData, output_path: str):
    ...
    for r in range(26, 36):
        ws[f"A{r}"] = None
    category_row = {1: 27, 2: 28, 3: 29, 4: 30, 5: 31, 6: 32}.get(contract.category)
    if category_row:
